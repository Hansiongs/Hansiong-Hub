...
local Locations = {
    ...
    ["Ocean"] = CFrame.new(43.05, 15, 2327.63),
}
...
local function ScanSecrets()
    ...
    for _, item in ipairs(allItems) do
        ...
        local isSecretTier = (info and (info.Rarity == "SECRET" or info.Tier == 7))
        local isWhitelisted = (FavoriteConfig.WhitelistId[item.Id] and variant and FavoriteConfig.WhitelistVariant[variant])
        if isSecretTier or isWhitelisted then
            totalSecretCount = totalSecretCount + 1
            local displayName = info.Name
            if isWhitelisted and variant then
                displayName = variant .. " " .. info.Name
            end

            if not grouped[displayName] then
                local newEntry = { Name = displayName, Count = 0 }
                table.insert(secretList, newEntry)
                grouped[displayName] = newEntry
            end
