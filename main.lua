...
function PurchaseMarketItem(id) return Net["RF/PurchaseMarketItem"]:InvokeServer(id) end
function RedeemCode(code) return Net["RF/RedeemCode"]:InvokeServer(code) end
function ConsumePotion(uuid, amount) return Net["RF/ConsumePotion"]:InvokeServer(uuid, amount or 1) end
if not Net then 
    return
end
...
function PurchaseTotem()
    local purchaseid = 5 --Luck Totem
    if Settings["Totem"] == "Mutation" then
        purchaseid = 8
    else
        purchaseid = 5
    end
...
function GetRods()
    ...
    Temporary["Enchant1"] = bestEnchant1
    Temporary["Enchant2"] = bestEnchant2
end
...
local function ScanSecrets()
    ...
    for _, item in ipairs(allItems) do
        ...
        if not isTarget then
            ...
            if variant and FavoriteConfig.WhitelistId[item.Id] and FavoriteConfig.WhitelistVariant[variant] then
                isTarget = true
                finalName = variant .. " " .. info.Name 
            end
        end
...
Net["RE/ObtainedNewFishNotification"].OnClientEvent:Connect(function(fishId, weight, itemData)
    Temporary["FishCatch"] = Temporary["FishCatch"] + 1
    Temporary["FishingCatch"] = Temporary["FishingCatch"] + 1
    
    local item = itemData and itemData.InventoryItem
    if not item then return end

    local fishInfo = DBFish[tostring(fishId)]
    local variant = item.Metadata and item.Metadata.VariantId 

    if (fishInfo and fishInfo.Tier == 7) or 
       (FavoriteConfig.WhitelistId[fishId] and variant and FavoriteConfig.WhitelistVariant[variant]) then
...
while Temporary["Running"] do
    ...
    if Settings["AutoFish"] then
        if Temporary["FishCatch"] > Settings["SellCount"] then
            ...
            if Temporary["BestRodId"] == 257 and GetCoin() > 1000000 then
                PurchaseTotem()
                task.wait(1)
            end
            ...
            if next(Settings["Quest"]) then
                if contains(Settings["Quest"], "DeepSea") then
                    ...
                    if not isObj1Done or not isObj2Done or not isObj3Done or not isObj4Done then
                        if not CanSecret then
                            DynamicLocation = "Kohana Volcano" 
                        elseif not isObj2Done or not isObj3Done then
                            DynamicLocation = "Sisyphus Statue" 
                        elseif not isObj1Done then
                            DynamicLocation = "Treasure Room" 
                        end
                    else
                ...
                if not contains(Settings["Quest"], "DeepSea") and contains(Settings["Quest"], "Jungle2025") then
                        ...
                        if not isJungle1Done or not isJungle2Done or not isJungle3Done then
                            ...
                            local TempleLevers = GetTempleLevers()
                            if not TempleLevers["Diamond Artifact"] then DynamicLocation = "Diamond Artifact"
                            elseif not TempleLevers["Crescent Artifact"] then DynamicLocation = "Crescent Artifact"
                            elseif not TempleLevers["Hourglass Diamond Artifact"] then DynamicLocation = "Hourglass Diamond Artifact"
                            elseif not TempleLevers["Arrow Artifact"] then DynamicLocation = "Arrow Artifact"
                            elseif not isJungle2Done then DynamicLocation = "Arrow Artifact" 
                            elseif not isJungle3Done then DynamicLocation = "Sacred Temple"
                            end
                        else
                            table.remove(Settings["Quest"], 1) 
                            DynamicLocation = nil
            ...
            if not contains(Settings["Quest"], "DeepSea") and not contains(Settings["Quest"], "Jungle2025") and not DynamicLocation then
                DynamicLocation = Settings["DefaultLocation"] --Default Location
            end

            if Temporary["Location"] ~= DynamicLocation and DynamicLocation then
                if DynamicLocation == "Ocean" then
